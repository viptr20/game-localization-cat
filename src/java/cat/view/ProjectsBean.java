...
public class ProjectsBean implements Serializable {
    ...
    private UploadedFile projectsCsvFile;
    private UploadedFile projectsXlsxFile;
    ...
    public UploadedFile getProjectsCsvFile() {
        return projectsCsvFile;
    }

    public void setProjectsCsvFile(UploadedFile projectsCsvFile) {
        this.projectsCsvFile = projectsCsvFile;
    }

    public UploadedFile getProjectsXlsxFile() {
        return projectsXlsxFile;
    }

    public void setProjectsXlsxFile(UploadedFile projectsXlsxFile) {
        this.projectsXlsxFile = projectsXlsxFile;
    }

    // ====== PrimeFaces 7 advanced upload listeners ======

    public void onProjectsCsvUpload(FileUploadEvent event) {
        this.projectsCsvFile = event.getFile();
    ...
    public void onProjectsXlsxUpload(FileUploadEvent event) {
        this.projectsXlsxFile = event.getFile();
        addMessage(FacesMessage.SEVERITY_INFO, "Excel selected", event.getFile().getFileName());
    }

    // ====== IMPORT CSV (id,name,source_lang,target_langs,status,description) ======

    public void uploadProjectsCsv() {
        System.out.println(">>> uploadProjectsCsv: file = " +
                (projectsCsvFile == null ? "null" : projectsCsvFile.getFileName()));

        if (projectsCsvFile == null
                || projectsCsvFile.getFileName() == null
                || projectsCsvFile.getFileName().trim().isEmpty()) {
        ...
        try (BufferedReader reader = new BufferedReader(
                new InputStreamReader(projectsCsvFile.getInputstream(), StandardCharsets.UTF_8))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                Project p = new Project();   // use default ctor and setters
                p.setId(id);
                p.setName(name);
                p.setSourceLang(sourceLang);
                p.setTargetLangs(targetLangs);
                p.setStatus(status);
            ...
            load();
            projectsCsvFile = null;
            addMessage(FacesMessage.SEVERITY_INFO,
                    "Import complete",
                    "Imported/updated projects: " + imported);
        } catch (Exception e) {
            e.printStackTrace();
            addMessage(FacesMessage.SEVERITY_ERROR,
                    "Import failed",
                    "Could not import projects CSV.");
        }
    }

    // ====== IMPORT XLSX (same column order as CSV) ======

    public void uploadProjectsXlsx() {
        System.out.println(">>> uploadProjectsXlsx: file = " +
                (projectsXlsxFile == null ? "null" : projectsXlsxFile.getFileName()));

        if (projectsXlsxFile == null
                || projectsXlsxFile.getFileName() == null
                || projectsXlsxFile.getFileName().trim().isEmpty()) {
            addMessage(FacesMessage.SEVERITY_WARN,
                    "No file selected",
        ...
        try (InputStream in = projectsXlsxFile.getInputstream()) {
            ...
            addMessage(FacesMessage.SEVERITY_ERROR,
                    "Import failed",
                    "Could not import projects XLSX.");
        } finally {
            if (workbook != null) {
                try {
                    workbook.close();
